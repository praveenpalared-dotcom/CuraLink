...
import { 
  ...
  CheckCircle, Plus, Check, Paperclip, Upload, X, Stethoscope, Activity, Network, Send
} from 'lucide-react';
import NotificationBell from '../components/NotificationBell';

export default function DoctorDashboard({ user, onLogout, onNavigate }) {
  ...
  // Research Collaboration State
  const [activeView, setActiveView] = useState('clinical'); // 'clinical' or 'research'
  const [collaborations, setCollaborations] = useState([]);
  const [newCollabMsg, setNewCollabMsg] = useState('');

  const fetchCollaborations = async () => {
    try {
      const res = await fetch(`/api/v1/research/collaborations/staff/1`);
      if (res.ok) setCollaborations(await res.json());
    } catch (e) { console.error(e); }
  };

  useEffect(() => {
    if (activeView === 'research') fetchCollaborations();
  }, [activeView]);

  const handleUpdateCollabStatus = async (id, status) => {
    await fetch(`/api/v1/research/collaborations/${id}/status?status=${status}`, { method: 'PUT' });
    fetchCollaborations();
  };
    ...
    <div className="min-h-screen bg-brand-bg flex flex-col">
        ...
        <div className="flex items-center gap-2 sm:gap-3">
          <div className="hidden sm:flex bg-brand-bg rounded-xl border border-brand-border p-1 mr-4">
            ...
            <button 
              ...
              className={`px-3 py-1 text-xs font-bold rounded-lg transition flex items-center gap-1.5 ${activeView === 'research' ? 'bg-brand-card shadow text-brand-accent border border-brand-accent/20' : 'text-brand-muted hover:text-brand-text'}`}
            >
              <Network className="w-3.5 h-3.5" /> Research Network
            </button>
          </div>
          <NotificationBell userType="staff" userId={1} />
          <div className="hidden md:block text-right">
            <span className="text-xs font-black block text-brand-text">{user?.name || "Dr. Richard"}</span>
      ...
      {activeView === 'research' && (
        <div className="flex-1 p-6 overflow-y-auto max-w-6xl mx-auto w-full space-y-6">
          <div className="p-8 bg-gradient-to-r from-brand-accent/10 via-brand-bg to-brand-bg border border-brand-accent/20 rounded-2xl">
            <h1 className="text-2xl font-black text-brand-text flex items-center gap-3">
              <Network className="w-6 h-6 text-brand-accent" /> Research Collaboration Hub
            </h1>
            <p className="text-sm text-brand-muted mt-2 max-w-2xl leading-relaxed">
              Connect with leading researchers and institutions worldwide. Share insights on clinical trials, coordinate on cross-functional studies, and advance global medical research.
            </p>
          </div>
          ...
          <div className="grid md:grid-cols-3 gap-6">
            ...
            <div className="md:col-span-2 space-y-4">
              <h3 className="font-bold text-lg text-brand-text border-b border-brand-border pb-2 flex items-center gap-2">
                <Users className="w-5 h-5 text-brand-accent" /> Incoming Collaboration Requests
              </h3>
              
              {collaborations.filter(c => c.status === 'pending').map(c => (
                <div key={c.id} className="p-5 bg-brand-card border border-brand-border rounded-xl shadow-sm hover:border-brand-accent/40 transition">
                  <div className="flex justify-between items-start mb-3">
                    <div>
                      <h4 className="font-bold text-brand-text">Researcher #{c.requester_id}</h4>
                      <p className="text-xs text-brand-muted font-mono">{new Date(c.created_at).toLocaleString()}</p>
                    </div>
                  ...
                  <div className="flex gap-2 justify-end">
                    <button 
                      onClick={() => handleUpdateCollabStatus(c.id, 'rejected')}
                      className="px-4 py-1.5 text-xs font-bold border border-red-200 text-red-600 hover:bg-red-50 rounded-lg transition"
                    >
                      Decline
                    </button>
                    <button 
                      onClick={() => handleUpdateCollabStatus(c.id, 'accepted')}
                      className="px-4 py-1.5 text-xs font-bold bg-brand-accent text-white hover:bg-brand-accent/90 rounded-lg shadow-sm transition"
                    >
                      Accept & Connect
                    </button>
                  </div>
              ...
              {collaborations.filter(c => c.status === 'pending').length === 0 && (
                <div className="text-center py-10 bg-brand-card/50 border border-dashed border-brand-border rounded-xl text-brand-muted text-sm">
                  No pending requests.
                </div>
              )}

              <h3 className="font-bold text-lg text-brand-text border-b border-brand-border pb-2 mt-8 flex items-center gap-2">
                <CheckCircle className="w-5 h-5 text-emerald-500" /> Active Collaborations
              </h3>
              
              {collaborations.filter(c => c.status === 'accepted').map(c => (
                <div key={c.id} className="p-4 bg-brand-card border border-emerald-500/20 rounded-xl shadow-sm flex justify-between items-center">
                  <div>
                    <h4 className="font-bold text-brand-text text-sm">Collaborator #{c.requester_id === 1 ? c.target_doctor_id : c.requester_id}</h4>
                    <p className="text-xs text-brand-muted font-mono mt-1">Connected since {new Date(c.updated_at).toLocaleDateString()}</p>
                  </div>
                  <button className="px-4 py-2 bg-brand-bg border border-brand-border rounded-lg text-xs font-bold hover:bg-brand-hover transition flex items-center gap-2">
            ...
            <div className="space-y-4">
              <div className="p-5 bg-brand-card border border-brand-border rounded-xl shadow-sm space-y-4">
                <h3 className="font-bold text-brand-text flex items-center gap-2 border-b border-brand-border pb-2">
                  <Search className="w-4 h-4 text-brand-teal" /> Discover Researchers
                </h3>
                <div className="p-3 bg-brand-bg rounded-lg border border-brand-border/60">
                  <h4 className="font-bold text-xs text-brand-text">Dr. Elena Rostova</h4>
                  <p className="text-[10px] text-brand-muted mb-2">Oncology & Clinical Trial Specialist • Stanford Med</p>
                  <button 
                    onClick={() => {
                      alert("Request sent successfully to Dr. Rostova!");
                    }}
                    className="w-full py-1.5 bg-brand-teal/10 text-brand-teal font-bold text-xs rounded-lg hover:bg-brand-teal/20 transition"
                  >
                    Connect
                  </button>
                </div>
                <div className="p-3 bg-brand-bg rounded-lg border border-brand-border/60">
                  <h4 className="font-bold text-xs text-brand-text">Dr. James Wilson</h4>
                  <p className="text-[10px] text-brand-muted mb-2">Genomics • Oxford Research</p>
                  <button 
                    onClick={() => {
                      alert("Request sent successfully to Dr. Wilson!");
                    }}
                    className="w-full py-1.5 bg-brand-teal/10 text-brand-teal font-bold text-xs rounded-lg hover:bg-brand-teal/20 transition"
                  >
                    Connect
