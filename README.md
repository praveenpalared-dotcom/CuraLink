...
- **Password-Based Authentication**: `POST /api/v1/auth/login` verifies patient and staff credentials against `argon2`-hashed passwords (via `passlib`) and issues a signed JWT (`python-jose`, HS256, 8-hour expiry) on success. Incorrect credentials return `401 Unauthorized` instead of granting access.
