...
export default function PatientDashboard({ onNavigate, userRole, setUserRole, sessionType, patientData, onLogout }) {
  ...
  useEffect(() => {
    ...
    if (['research_feed', 'notifications', 'saved_searches', 'health_insights'].includes(activeTab)) {
      fetchResearchData();
    }
  }, [activeTab]);
  ...
  const fetchResearchData = async () => {
    try {
      const pId = patientData?.id || 1;
      const tRes = await fetch('/api/v1/research/trials');
      if (tRes.ok) setTrials(await tRes.json());
      
      const pRes = await fetch('/api/v1/research/papers');
      if (pRes.ok) setPapers(await pRes.json());

      const sRes = await fetch(`/api/v1/research/saved_searches/patient/${pId}`);
      if (sRes.ok) setSavedSearches(await sRes.json());

      const nRes = await fetch(`/api/v1/notifications/patient/${pId}`);
      if (nRes.ok) setSmartNotifications(await nRes.json());
    } catch (e) {
      console.error(e);
    }
  };
  ...
  const handleClearAll = () => setSmartNotifications([]);
  const handleAddSearch = async (search) => {
    await fetch('/api/v1/research/saved_searches', {
      method: 'POST', headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ ...search, patient_id: patientData?.id || 1 })
    });
    fetchResearchData();
  };
  const handleDeleteSearch = (id) => setSavedSearches(savedSearches.filter(s => s.id !== id));
  ...
  return (
    <div className="min-h-screen bg-brand-bg flex flex-col md:flex-row font-sans">
      ...
      <aside className="w-full md:w-64 bg-brand-card border-r border-brand-border flex flex-col shrink-0 z-20 h-screen">
        <div onClick={() => setActiveTab('home')} className="p-4 border-b border-brand-border flex items-center gap-2 cursor-pointer hover:opacity-85 transition">
